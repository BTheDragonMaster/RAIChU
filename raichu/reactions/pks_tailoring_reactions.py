...
RECENT_REDUCTION_MMAL_CHIRAL_C = GroupDefiner(
    "recent_reduction_mmal_chiral_c", r"CCC(C(=O)S)C", 2
)
...
RECENT_REDUCTION_CC_SHIFTED = BondDefiner(
    "recent_reduction_C-C_shifted", r"CC(O)CC(S)=O", 0, 1
)
...
RECENT_REDUCTION_TOP_C = GroupDefiner(
    "recent_reduction_top_c", r"[H]C(C)=C([H])C(S)=O", 5
)
RECENT_REDUCTION_TOP_H = GroupDefiner(
    "recent_reduction_top_h", r"[H]C(C)=C([H])C(S)=O", 4
)
# RECENT_REDUCTION_TOP_METHYL = GroupDefiner('recent_reduction_top_methyl', r'[H]\C(C)=C(\C)C(S)=O', 4)
RECENT_REDUCTION_TOP_METHYL = GroupDefiner(
    "recent_reduction_top_methyl", r"[H]C(C)=C(C)C(S)=O", 4
)
RECENT_REDUCTION_TOP_METHOXY = GroupDefiner(
    "recent_reduction_top_methoxy", r"[H]C(C)=C(OC)C(S)=O", 4
)
RECENT_REDUCTION_BOTTOM_C = GroupDefiner(
    "recent_reduction_bottom_c", r"[H]C(C)=C([H])C(S)=O", 2
)
RECENT_REDUCTION_BOTTOM_H = GroupDefiner(
    "recent_reduction_bottom_h", r"[H]C(C)=C([H])C(S)=O", 0
)

RECENT_REDUCTION_SHIFTED_TOP_C = GroupDefiner(
    "recent_reduction_shifted_top_c", r"[H]\C(C)=C(\[H])CC(S)=O", 5
)
RECENT_REDUCTION_SHIFTED_TOP_H = GroupDefiner(
    "recent_reduction_shifted_top_h", r"[H]\C(C)=C(\[H])CC(S)=O", 4
)
RECENT_REDUCTION_SHIFTED_BOTTOM_C = GroupDefiner(
    "recent_reduction_shifted_bottom_c", r"[H]\C(C)=C(\[H])CC(S)=O", 2
)
RECENT_REDUCTION_SHIFTED_BOTTOM_H = GroupDefiner(
    "recent_reduction_shifted_bottom_h", r"[H]\C(C)=C(\[H])CC(S)=O", 0
)
RECENT_REDUCTION_SHIFTED_BOTTOM_METHYL = GroupDefiner(
    "recent_reduction_shifted_bottom_methyl", r"[H]\C(CC(S)=O)=C(\C)CC", 7
)
S_KR = GroupDefiner("C1 atom before KR reaction", r"SC(C)=O", 0)
ER_MMAL_CARBON = GroupDefiner(
    "Chiral carbon atom after enoylreduction of mmal", r"SC(=O)C(C)CC", 3
)
ER_S_CARBON = GroupDefiner(
    "S-carbon atom after enoylreduction of mmal", r"SC(=O)C(C)CC", 1
)
HYDROXYL_GROUP_TWO_MODULES_UPSTREAM_ALPHA_WITH_DOUBLE_BOND = BondDefiner(
    "hydroxyl_group_two_module_upstream_alpha_with_double_bond",
    r"[H]OC\C=C\CCC(S)=O",
    1,
    2,
)
HYDROXYL_GROUP_TWO_MODULES_UPSTREAM_BETA_WITH_DOUBLE_BOND = BondDefiner(
    "hydroxyl_group_two_module_upstream_beta_with_double_bond",
    r"[H]OCC\C=C\CCC(S)=O",
    1,
    2,
)
HYDROXYL_GROUP_TWO_MODULES_UPSTREAM_ALPHA_WITH_DOUBLE_BOND_SHIFTED = BondDefiner(
    "hydroxyl_group_two_module_upstream_alpha_with_double_bond_shifted",
    r"[H]O\C=C\CCCC(S)=O",
    1,
    2,
)
HYDROXYL_GROUP_TWO_MODULES_UPSTREAM_BETA_WITH_DOUBLE_BOND_SHIFTED = BondDefiner(
    "hydroxyl_group_two_module_upstream_beta_with_double_bond_shifted",
    r"[H]OC\C=C\CCCC(S)=O",
    1,
    2,
)
HYDROXYL_GROUP_TWO_MODULES_UPSTREAM_ALPHA = BondDefiner(
    "hydroxyl_group_two_module_upstream_alpha", "[H]OCCCCCC(S)=O", 1, 2
)
HYDROXYL_GROUP_TWO_MODULES_UPSTREAM_BETA = BondDefiner(
    "hydroxyl_group_two_module_upstream_beta", "[H]OCCCCCCC(S)=O", 1, 2
)


def ketoreduction(
    chain_intermediate: Structure, kr_type: KRDomainSubtype
    ...
    if kr_type.name != "C2":
        ...
        # If the type of KR domain is not known, the chirality of the carbonyl carbon is set to None
            ...
            raise ValueError(
                f"KR domain of type {kr_type.name} is not supported by RAIChU or does not exist"
            )
    ...
    if chiral_c and kr_type.name != "UNKNOWN":
        ...
        for atom in chiral_c.neighbours:
            if (
                ...
                and atom != c_3
                and atom.type != "H"
                and not atom.annotations.in_central_chain
            ):
...
def dehydration(
    chain_intermediate: Structure, chirality=None
) -> Tuple[Structure, bool]:
    """
    ...
    if chirality:
        main_chain_top_c = chain_intermediate.get_atom(
            find_atoms(RECENT_REDUCTION_TOP_C, chain_intermediate)[0]
        )
        main_chain_bottom_c = chain_intermediate.get_atom(
            find_atoms(RECENT_REDUCTION_BOTTOM_C, chain_intermediate)[0]
        )
        ...
        if not main_chain_top_h:
            main_chain_top_h = [
                neighbour
                for neighbour in c1.neighbours
                if not neighbour.annotations.in_central_chain
            ]
        ...
        if not main_chain_bottom_h:
            ...
            main_chain_bottom_h = chain_intermediate.get_atom(
                [
                    neighbour
                    for neighbour in c2.neighbours
                    if not neighbour.annotations.in_central_chain
                ][0]
            )
        else:
        ...
        if (
            not main_chain_top_h
            or not main_chain_bottom_h
            or not main_chain_top_c
            or not main_chain_bottom_c
        ):
            return chain_intermediate, True

        if chirality == "E":
            double_bond.chiral_dict = {
                main_chain_top_c: {
                    main_chain_bottom_c: "trans",
                    main_chain_bottom_h: "cis",
                },
                main_chain_top_h: {
                    main_chain_bottom_c: "cis",
                    main_chain_bottom_h: "trans",
                },
                main_chain_bottom_c: {
                    main_chain_top_c: "trans",
                    main_chain_top_h: "cis",
                },
                main_chain_bottom_h: {
                    main_chain_top_c: "cis",
...
def enoylreduction(
    chain_intermediate: Structure, er_subtype: ERDomainSubtype
) -> Tuple[Structure, bool]:
    """
    ...
    if len(er_carbons) == 1 and er_subtype.name != "UNKNOWN":
        ...
        if (
            len(er_carbon.get_neighbours("H")) == 1
            and len(er_carbon.get_neighbours("C")) == 3
        ):
            ...
            for carbon_neighbour in er_carbon.get_neighbours("C"):
                if (
                    carbon_neighbour.annotations.in_central_chain
                    and carbon_neighbour != s_carbon
                ):
            ...
            if er_subtype.name == "S" and has_s_chirality:
                ...
                raise ValueError(
                    f"RAIChU does not support ER domain subtype {er_subtype.name}"
                )
...
def methylation_with_rest(target_atom, structure: Structure) -> Structure:
    """
    Returns the structure thats methylated with a unknown rest at the target atom.
    ...
    methyl_group.add_attributes(ATTRIBUTES, boolean=True)
    carbon = methyl_group.atoms[0]
    hydrogen_1 = methyl_group.atoms[2]
    bond_1 = carbon.get_bond(hydrogen_1)
...
def find_OH_two_modules_upstream(structure: Structure):
    ...
    if not bonds:
        bonds = find_bonds(
            HYDROXYL_GROUP_TWO_MODULES_UPSTREAM_BETA_WITH_DOUBLE_BOND, structure
        )
    if not bonds:
        bonds = find_bonds(
            HYDROXYL_GROUP_TWO_MODULES_UPSTREAM_BETA_WITH_DOUBLE_BOND_SHIFTED, structure
        )
    return bonds
...
def beta_branching_cassette(structure: Structure) -> Tuple[Structure, bool]:
    """
    Performs the reaction of a beta branching casette on the PKS chain intermediate, returns
    the reaction product as a PIKAChU Structure object
    ...
    if beta_c:
        if beta_c[0].has_neighbour("H"):
            structure = methylation_with_rest(beta_c[0], structure)
            return structure, True
        else:
            return structure, False
    else:
...
def gamma_beta_dehydratase(
    ...
    if chirality:
        ...
        main_chain_top_c = chain_intermediate.get_atom(
            find_atoms(RECENT_REDUCTION_SHIFTED_TOP_C, chain_intermediate)[0]
        )
        main_chain_bottom_c = chain_intermediate.get_atom(
            find_atoms(RECENT_REDUCTION_SHIFTED_BOTTOM_C, chain_intermediate)[0]
        )
        main_chain_top_h = find_atoms(
            RECENT_REDUCTION_SHIFTED_TOP_H, chain_intermediate
        )
        main_chain_bottom_h = chain_intermediate.get_atom(
            find_atoms(RECENT_REDUCTION_SHIFTED_BOTTOM_H, chain_intermediate)[0]
        )
        if not main_chain_bottom_h:
            main_chain_bottom_h = chain_intermediate.get_atom(
                find_atoms(RECENT_REDUCTION_SHIFTED_BOTTOM_METHYL, chain_intermediate)[
                    0
                ]
            )
        else:
