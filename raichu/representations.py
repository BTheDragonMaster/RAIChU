...
class ClusterRepresentation:
    ...
    def write_cluster(self, out_dir):
        ...
        with open(cluster_out, 'w') as out:
            out.write("gene_name\tmodule_nr\tmodule_type\tmodule_subtype\tmodule_substrate\tnr_iterations\tdomain_type\tdomain_subtype\tdomain_name\tdomain_used\tdomain_active\n")
            for i, module in enumerate(self.modules):
                for domain in module.domains:
                    out.write(f"{domain.gene_name}\t{i}\t{module.type}\t{module.subtype}\t{module.substrate}\t{module.iterations}\t{domain.type}\t{domain.subtype}\t{domain.name}\t{domain.used}\t{domain.active}\n")
        ...
        with open(in_cluster, 'r') as cluster:
            ...
            for line in cluster:
                ...
                if line:
                    ...
                    gene_name, module_nr, module_type, module_subtype, substrate, iterations, domain_type, domain_subtype, domain_name, domain_used, domain_active = line_info_cleaned
            ...
            for module_nr, module_data in modules.items():
                module = ModuleRepresentation(module_data["type"], module_data["subtype"], module_data["substrate"], module_data["domains"], module_data["iterations"])
